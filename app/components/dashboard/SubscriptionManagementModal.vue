    ...
    <div class="bg-ui-surface border border-ui-border p-6 rounded-2xl shadow-2xl max-w-md w-full">
      ...
      <div v-else-if="subscription && subscription.active" class="space-y-6">
        ...
        <div class="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-100 rounded-xl">
          ...
          <div>
            <p class="text-xs font-bold text-emerald-800 uppercase tracking-wider">Statut</p>
            <p class="font-medium text-emerald-900">
              {{ subscription.cancel_at_period_end ? 'Se termine bientôt' : 'Actif & Renouvellement auto' }}
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="p-4 bg-ui-surface-muted/50 rounded-xl border border-ui-border">
            <p class="text-xs text-ui-content-muted font-medium mb-1">Prochaine facture</p>
