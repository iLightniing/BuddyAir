...
const { displayedBalances } = useBalanceAnimation(toRef(props, 'balances'))
</script>

<template>
  <div v-if="account" class="sticky top-14 z-30 bg-ui-surface/80 backdrop-blur-md border border-ui-border rounded-md p-2 shadow-sm transition-all duration-300">
    <div class="grid grid-cols-1 xl:grid-cols-3 items-center gap-4">
      ...
      <div class="flex items-center gap-2 w-full xl:w-auto justify-self-start">
        <div class="relative group">
          <NuxtLink to="/dashboard/accounts" class="w-10 h-10 flex items-center justify-center bg-ui-surface hover:bg-ui-surface-muted border border-ui-border rounded-md text-ui-content transition-all shadow-sm shrink-0 hover:scale-105 active:scale-95">
        ...
        <div class="relative group">
          <button @click="emit('add')" class="w-10 h-10 flex items-center justify-center bg-ui-surface hover:bg-ui-surface-muted border border-ui-border rounded-md text-emerald-600 transition-all shadow-sm shrink-0 hover:scale-105 active:scale-95 cursor-pointer">
        ...
        <div class="relative group">
          <button @click="emit('show-analytics')" class="w-10 h-10 flex items-center justify-center bg-ui-surface hover:bg-ui-surface-muted border border-ui-border rounded-md text-purple-600 transition-all shadow-sm shrink-0 hover:scale-105 active:scale-95 cursor-pointer">
        ...
        <div v-if="account.account_group !== 'credit'" class="relative group">
          <NuxtLink :to="`/dashboard/schedule?account=${account.id}`" class="w-10 h-10 flex items-center justify-center bg-ui-surface hover:bg-ui-surface-muted border border-ui-border rounded-md text-blue-600 transition-all shadow-sm shrink-0 hover:scale-105 active:scale-95">
        ...
        <div v-if="account.account_group === 'credit'" class="relative group">
          <button @click="emit('show-amortization')" class="w-10 h-10 flex items-center justify-center bg-ui-surface hover:bg-ui-surface-muted border border-ui-border rounded-md text-ui-content transition-all shadow-sm shrink-0 hover:scale-105 active:scale-95">
        ...
        <div v-if="selectedTransactions.length > 0" class="relative" ref="optionsDropdownRef">
          <button @click="showOptions = !showOptions" class="w-10 h-10 flex items-center justify-center bg-ui-surface hover:bg-ui-surface-muted border border-ui-border rounded-md text-ui-content transition-all shadow-sm relative hover:scale-105 active:scale-95">
          ...
          <div v-if="showOptions" class="absolute top-full left-0 mt-2 w-48 bg-ui-surface border border-ui-border rounded-md shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div class="p-1 space-y-0.5">
              <button @click="emit('bulk-point', true); showOptions = false" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-ui-content hover:bg-ui-surface-muted rounded-sm text-left">
                <Icon name="lucide:check" class="w-3.5 h-3.5 text-emerald-500" /> Pointer
              </button>
              <button @click="emit('bulk-point', false); showOptions = false" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-ui-content hover:bg-ui-surface-muted rounded-sm text-left">
                <Icon name="lucide:rotate-ccw" class="w-3.5 h-3.5 text-orange-500" /> Dépointer
              </button>
              <button @click="emit('bulk-delete'); showOptions = false" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50 rounded-sm text-left">
              ...
              <button @click="emit('export'); showOptions = false" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-ui-content hover:bg-ui-surface-muted rounded-sm text-left">
                <Icon name="lucide:file-spreadsheet" class="w-3.5 h-3.5" /> CSV
              </button>
              <button @click="emit('export-json'); showOptions = false" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-ui-content hover:bg-ui-surface-muted rounded-sm text-left">
  ...
  <div v-if="searchQuery || selectedTransactions.length === 0" class="flex flex-col md:flex-row items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300 px-1 mt-4">
     ...
     <div class="flex items-center gap-3 w-full md:w-auto">
        <div class="w-40 shrink-0">
          <UiDropdown :model-value="filterType" @update:model-value="emit('update:filterType', $event)" :options="typeOptions" class="!rounded-md" />
        </div>
        <div class="w-40 shrink-0">
          <UiDropdown :model-value="filterStatus" @update:model-value="emit('update:filterStatus', $event)" :options="statusOptions" class="!rounded-md" />
        </div>
     </div>
  </div>
</template>