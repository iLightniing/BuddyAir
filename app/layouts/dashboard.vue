  ...
  <div class="flex min-h-screen">
    ...
    <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-ui-surface/90 backdrop-blur-xl border-t border-ui-border pb-safe">
      <div class="flex justify-around items-center h-16">
        <NuxtLink v-for="link in mobileLinks" :key="link.to" :to="link.to" :class="['flex flex-col items-center justify-center w-full h-full gap-1 transition-colors', (link.exact ? route.path === link.to : route.path.startsWith(link.to)) ? 'text-blue-500' : 'text-ui-content-muted hover:text-ui-content']">
